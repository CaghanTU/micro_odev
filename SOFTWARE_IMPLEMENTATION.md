...
| `logger.cpp` | Stores the last 10 events in a RAM ring buffer with boot-time timestamps. |
...
The system uses one DHT22 sensor connected to GPIO23. Normal sensor sampling occurs every 2 seconds. If the first read fails, the firmware schedules one short non-blocking retry after 250 ms. A failed sample attempt is counted only if both the normal read and the retry fail.
...
The firmware uses a RAM-only ring buffer with a capacity of 10 entries. No LittleFS or flash-based logging is used in this version.
...
The physical ESP32 board, DHT22 sensor, LCD, keypad, MOSFET drivers, fan, humidifier, heater, Peltier module, and buzzer have not yet been tested with this firmware. Physical hardware testing will be performed when the assembled system is available.
